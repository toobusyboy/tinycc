...
void dec()//tested
{
	...
	else if(token==rsv_extern)//外部变量声明
	{
		...
		if(!match(semicon))
		{
			if(token==rsv_extern||token==rsv_void||token==rsv_int||  \
			token==rsv_char||token==rsv_string)//丢失分号
			{
...
void funtail(symbol dec_type,string dec_name)
{
	...
	else if(token==ident||token==rsv_if||token==rsv_while
	        ||token==rsv_return||token==rsv_break||token==rsv_continue
	...
	else if(token==rsv_void||token==rsv_int||
	        token==rsv_char||token==rsv_string)//其他的作为声明处理
	{
		synterror(semiconlost,-1);
		BACK
		return ;
...
void varlist(symbol dec_type)//tested
{
	...
	else if(token==rsv_int||token==rsv_void||
			token==rsv_char||token==rsv_string)//忘记分号
	{
		synterror(semiconlost,-1);
		BACK
	}
...
void statement(int & var_num ,int& level,int lopId,int blockAddr)
{
	...
	case rsv_continue:
		p("continue语句",0);
		nextToken();
		if(token==ident||token==rsv_while||token==rsv_if||
		   token==rsv_return||token==rsv_break||token==rsv_continue||
		   token==rsv_in||token==rsv_out||token==rbrac)
		{
			synterror(semiconlost,-1);
...
void ifstat(int &var_num,int & level,int lopId,int blockAddr)
{
	...
	if(!match(rparen))
	{
		if(token==lbrac||token==semicon||token==rsv_while||token==rsv_if||token==rsv_return
			||token==rsv_break||token==rsv_continue||token==rsv_in||token==rsv_out
			||token==ident||token==rsv_void||token==rsv_int||token==rsv_char||token==rsv_string)
		{
			synterror(staterparenlost,-1);
			BACK
